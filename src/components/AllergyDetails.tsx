...
const AllergyDetails = () => {
  ...
  const [allergyData, setAllergyData] = useState<{ patientId: string; name: string; severity: string; reaction: string; treatment: string; }>({
    name: '',
    severity: '',
    reaction: '',
    treatment: '',
  ...
  const handleAddProfile = async (e: FormEvent) => {
    ...
    try {
      ...
      setLoading(false);
        ...
        console.error('Error in createAllergy:', err);
        toast.error('Error in CreateAllergy. Please try again later.', {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 3000, 
        });
        setLoading(false);
  ...
  const formatDateTime = (date: any) => {
   ...
   return (
    <>
      ...
      {isCardOpen && (
        <>
            {getAllergy != null && getAllergy?.length > 0 ? (
            <>
            {getAllergy?.map((user, index) => (
