...
const LabTestDetails = (props) => {
    ...
    try {
      ...
      if (record) {    
        ...
        DIDs.map(async (did) => {
          ...
          console.log('Send record status in labTestRecord', status)
        })
      );
      } else {
        toast.error('Failed to create labTest record', {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 3000, 
          });
          setLoading(false);
        throw new Error('Failed to create labTest record');       
      }
  ...
  const writeProfileToDwn = async (labTestDetails: any) => {
    ...
    try {
      const labTestProtocol = profileProtocolDefinition;
      const { record, status } = await web5.dwn.records.write({
        data: {...labTestDetails, timestamp: timestamp},
        message: {
          protocol: labTestProtocol.protocol,
          protocolPath: 'patientProfile/labTestRecord',
          schema: labTestProtocol.types.labTestRecord.schema,
          recipient: myDid,
          parentId: parentId,
          contextId: contextId,
        },
      ...
      console.log('Successfully wrote labTest details to DWN:', record);
      toast.success('LabTest Details written to DWN', {
        position: toast.POSITION.TOP_RIGHT,
        autoClose: 3000, 
      });
      return record;
    } catch (err) {
      console.error('Failed to write labTest details to DWN:', err);
      toast.error('Failed to write labTest details to DWN. Please try again later.', {
        position: toast.POSITION.TOP_RIGHT,
        autoClose: 3000,
      });
    }
   ...
   const fetchLabTestDetails = async () => {
    ...
    try {
      ...
      console.log('LabTest Details:', response);
  
      if (response.status.code === 200) {
        const labTestDetails = await Promise.all(
        ...
        toast.error('Failed to fetch labTest details', {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 3000,
        });
      }
      ...
      console.error('Error in fetchLabTestDetails:', err);
      toast.error('Error in fetchLabTestDetails. Please try again later.', {
        position: toast.POSITION.TOP_RIGHT,
        autoClose: 5000,
      });
      setFetchDetailsLoading(false);
   ...
   const shareLabTestDetails = async (recordId: string) => {
    ...
    try {
      ...
      if (response.records && response.records.length > 0) {
        ...
        toast.success('Successfully shared labTest record', {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 3000,
        });
        ...
        toast.error('Failed to share labTest record', {
          position: toast.POSITION.TOP_RIGHT,
          autoClose: 3000,
        });
      }
  ...
  const deleteLabTestDetails = async (recordId) => {
    try {
      const response = await web5.dwn.records.query({
        message: {
          filter: {
      ...
      if (response.records && response.records.length > 0) {
        ...
        if (deleteResult.status.code === 202) {
          console.log('LabTest Details deleted successfully');
          toast.success('LabTest Details deleted successfully', {
            position: toast.POSITION.TOP_RIGHT,
            autoClose: 3000, 
          });
          setUserDetails(prevLabTestDetails => prevLabTestDetails.filter(message => message.recordId !== recordId));
        } else {
      ...
      console.error('Error in deleteLabTestDetails:', error);
    }
  };
                ...
                >
                  <div
                    ...
                    <div
                      ...
                      ">        
                        <div className="flex flex-row justify-between ">
                          <h2 className="text-xl font-semibold mb-4">Share LabTest Details</h2>
                          <div className="flex justify-end">
                            <button
                    ...
                    <div className="-mx-4 flex flex-wrap">
                      ...
                      <div className="w-full px-4">
                        <button 
                          type="button"
                          onClick={() => shareLabTestDetails(userDetails.recordId)}
      ...
      {isCardOpen && (
        <>
          ...
          <div className="flex w-full" key={index}>
            ...
            <div className='w-1/3 mb-5'>
              <span className="text-xl">Refrence Range</span>
              <h4 className="text-xl mt-1 font-medium text-black dark:text-white">
                {user.referenceRange}
              </h4>
            </div>
                    ...
                    <div className="mt-4 flex justify-end">
                      ...
                      <button
                        onClick={() => {
                          ...
                          deleteLabTestDetails(user.recordId);
                        }}
