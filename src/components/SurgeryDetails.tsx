...
const SurgeryDetails = () => {
  ...
  const [surgeryData, setSurgeryData] = useState<{ surgery: string; patientId: string; hospital: string; status: string; }>({
    surgery: '',
    patientId: '',
    hospital: '',
    status: '',
  ...
  const getSurgery = useQuery(api.healthRecord.getSurgeryDetails, { patientId: userId });
  const createSurgery = useMutation(api.healthRecord.createSurgeryDetails);
  // const updateSurgery = useMutation(api.healthRecord.updateSurgeryDetails);
  const deleteSurgery = useMutation(api.healthRecord.deleteSurgeryDetails);
  ...
  const handleAddProfile = async (e: FormEvent) => {
    ...
    surgerydata.append('surgery', surgeryData.surgery);
    surgerydata.append('hospital', surgeryData.hospital);
    surgerydata.append('status', surgeryData.status);

    setLoading(false);
  
    try {
      await createSurgery({ ...surgeryData, patientId: userId })
      ...
      setLoading(false);
        ...
        console.error('Error in creating record:', err);
        toast.error('Error in creating record. Please try again.', {
          position: toast.POSITION.TOP_RIGHT,
  ...
  const deleteSurgeryDetails = async (userId: any) => {
    try {
      await deleteSurgery({ id: userId});
  ...
  const formatDateTime = (date: any) => {
   ...
   return (
    <>
                ...
                >
                  ...
                  >
                    ...
                    <div className="w-full xl:w-3/5">
                        ...
                        <div className={`relative ${surgeryData.surgery ? 'bg-light-blue' : ''}`}>
                        <input
                          type="text"
                          name="surgery"
                          required
                          value={surgeryData.surgery}
                      ...
                      <div className="w-full xl:w-3/5">
                        ...
                        <div className={`relative ${surgeryData.hospital ? 'bg-light-blue' : ''}`}>
                        <input
                            type='text'
                              name="hospital"
                    ...
                    <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                      ...
                      <div className="w-full xl:w-3/5">
                        ...
                        <div className={`relative ${surgeryData.status ? 'bg-light-blue' : ''}`}>
                        <textarea
                          name="status"
                          row={3}
                          value={surgeryData.status}
                          required
                          onChange={handleInputChange}
                          placeholder="Notes"
                          className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-2 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus-border-primary"/>
      ...
      {isCardOpen && (
        <>
            {getSurgery != null && getSurgery?.length > 0 ? (
            <>
            {getSurgery?.map((user, index) => (
